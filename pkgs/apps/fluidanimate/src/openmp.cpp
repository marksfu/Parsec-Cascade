...
void RebuildGridZ(int ck) {
	int i = ck * ny * nx;
	for(int cj = 0; cj < ny; ++cj) {
		for(int ci = 0; ci < nx; ++ci, ++i) {
...
void ComputeForces()
{
	...
	for (int foo = 0; foo <=2; foo += 2) {
		#pragma omp parallel for
		for(int ck = foo; ck < nz; ck += 4) {
			ComputeForceA(ck);
			if ((ck+1)<nz) {
				ComputeForceA(ck+1);
			}
